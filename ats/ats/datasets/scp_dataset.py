...
from ats.utils import HDF5ScpLoader
from ats.utils import NpyScpLoader
...
class ArtSCPDataset(Dataset):
    ...
    def __init__(
        ...
        """
        # load scp as lazy dict
        art_loader = _get_feats_scp_loader(feats_scp)
        art_keys = list(art_loader.keys())

        self.art_loader = art_loader
        self.utt_ids = art_keys
        self.return_utt_id = return_utt_id
        self.allow_cache = allow_cache
...
class ArtSCPMultDataset(Dataset):
    """PyTorch compatible mel dataset based on kaldi-stype scp files."""
    ...
    def __getitem__(self, idx):
        ...
        utt_id = self.utt_ids[idx]
        art = np.load(self.input_paths[idx])
