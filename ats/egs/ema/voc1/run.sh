...
pretrain2="" # checkpoint path to load pretrained parameters
            # (e.g. ../../jsut/<path>/<to>/checkpoint-400000steps.pkl)
...
train_set="train_no_dev" # name of training data directory
...
train_dumpdirs=""
if [ ! -z "${train_sets}" ]; then
    for d in ${train_sets}; do
        train_dumpdirs+="${dumpdir}/${d}/norm "
    done
    train_dumpdirs=${train_dumpdirs::-1}
    train_set=${train_sets%% *}
fi
dev_dumpdirs=""
if [ ! -z "${dev_sets}" ]; then
    for d in ${dev_sets}; do
        dev_dumpdirs+="${dumpdir}/${d}/norm "
    done
    dev_dumpdirs=${dev_dumpdirs::-1}
    dev_set=${dev_sets%% *}
fi
if [ "${stage}" -le 2 ] && [ "${stop_stage}" -ge 2 ]; then
    ...
    if [ "${n_gpus}" -gt 1 ]; then
        train="python -m ats.distributed.launch --nproc_per_node ${n_gpus} -c ats-train"
    else
        train="ats-train"
    fi
    ...
    ${cuda_cmd} --gpu "${n_gpus}" "${expdir}/train.log" \
        ${train} \
            ...
            --train-dumpdirs "${train_dumpdirs}" \
            --dev-dumpdirs "${dev_dumpdirs}" \
            --outdir "${expdir}" \
            --resume "${resume}" \
            --pretrain "${pretrain}" \
            --pretrain2 "${pretrain2}" \
...
if [ "${stage}" -le 3 ] && [ "${stop_stage}" -ge 3 ]; then
    ...
    outdir="${expdir}/wav/best_mel_ckpt"
    pids=()
    ...
    (
        ...
        ${cuda_cmd} --gpu "${n_gpus}" "${outdir}/${name}/decode.log" \
            ats-decode \
                ...
                --checkpoint "${expdir}/best_mel_ckpt.pkl" \
                --outdir "${outdir}/${name}" \
                --verbose "${verbose}"
